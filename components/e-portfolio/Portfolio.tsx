...
import { useConsumerBadgesList, useTest } from "@/components/api/OkutepApi"
...
export const Portfolio = () => {
  ...
  const { testData, isTestLoading, isTestError } = useTest()
  if (testData) {
    console.log(testData)
  }

  if (isTestLoading) return <Spinner/>
  if (isTestError) return <div>failed to load</div>
  return (
    <>
    OK
    </>
